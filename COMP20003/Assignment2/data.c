...
..Project: COMP20003 Assignment 2
    data.c :
            = the implementation of data module for Assignment 2
--------------------------------------------------------------------------- */
...
int cmpSuburbName(void* key, void *a, int *bit_count) {
    // Assert that the pointers are valid
    assert(a != NULL);

    suburb_t *sub = (suburb_t *)a;

    return compare_strings((char*)key, sub->suburb_name, bit_count);
}
...
/* Retrieves the key of the subrub */
char *get_key(suburb_t *s) {
    return s->suburb_name;
}
